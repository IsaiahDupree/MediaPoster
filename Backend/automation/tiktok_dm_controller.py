#!/usr/bin/env python3
"""
TikTok DM Controller - Safari automation for TikTok Direct Messages
====================================================================
Implements TIKTOK-DM-001 through TIKTOK-DM-005 for managing TikTok DMs
...
try:
    ...
    try:
        ...
        logger.warning("Session manager not available for TikTok DM controller")
...
@dataclass
class TikTokMessage:
    """A single message in a TikTok DM conversation."""
...
class TikTokConversation:
    ...
    is_read: bool = True
    is_muted: bool = False
    messages: List[TikTokMessage] = field(default_factory=list)
...
class TikTokDMController:
    """
    Safari-based DM controller for TikTok.

    Provides methods for navigating to the TikTok inbox, reading and sending
    messages, starting new conversations, and sending media/stickers.
    ...
    TIKTOK_INBOX_URL = "https://www.tiktok.com/messages"
    TIKTOK_LOGIN_URL = "https://www.tiktok.com/login"

    _instance: Optional["TikTokDMController"] = None

    @classmethod
    def get_instance(cls, dev_mode: bool = False) -> "TikTokDMController":
    ...
    def __init__(self, dev_mode: bool = False):
        ...
        # In-memory state --------------------------------------------------
        self.conversations: Dict[str, TikTokConversation] = {}
        ...
        logger.info("TikTokDMController initialized (dev_mode={})", dev_mode)
    ...
    def navigate_to_inbox(self) -> Dict[str, Any]:
        """
        Navigate Safari to the TikTok DM inbox.

        Returns:
            Dict with ``success`` flag and current URL.
        """
        logger.info("Navigating to TikTok inbox")
        success = self._navigate_to(self.TIKTOK_INBOX_URL)
        if success:
            return {"success": True, "url": self.TIKTOK_INBOX_URL}
        return {"success": False, "error": "Failed to navigate to TikTok inbox"}

    def authenticate(self) -> Dict[str, Any]:
        """
        Verify that the user is authenticated on TikTok in Safari.
        ...
        """
        logger.info("Checking TikTok authentication status")
        ...
        if self.session_manager:
            try:
                logged_in = self.session_manager.require_login(Platform.TIKTOK)
                if logged_in:
                    self.is_authenticated = True
                    return {"authenticated": True}
                return {"authenticated": False, "error": "Not logged in to TikTok"}
            except Exception as e:
                logger.warning("Session manager check failed: {}", e)

        # Fallback: navigate and inspect the page
        self._navigate_to(self.TIKTOK_BASE_URL)
        script = '''
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {
                            ...
                            if (profileIcon || uploadIcon) {
                                return JSON.stringify({authenticated: true});
                            }
                            var loginBtn = document.querySelector('[data-e2e=\\"top-login-button\\"]');
                            if (loginBtn) {
                                return JSON.stringify({authenticated: false, reason: 'login_button_visible'});
                            }
    ...
    def get_inbox(self) -> Dict[str, Any]:
        """
        Retrieve the TikTok DM inbox (list of conversations with metadata).

        Returns:
            Dict with ``success``, ``count``, ``unread_count``, and
            ``conversations`` list.
        """
        logger.info("Fetching TikTok DM inbox")
        ...
        self._navigate_to(self.TIKTOK_INBOX_URL)
        script = '''
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {
                            var conversations = [];
                            var items = document.querySelectorAll('[class*=\\"ConversationList\\"] > div, [data-e2e=\\"inbox-conversation\\"]');
                            for (var i = 0; i < items.length; i++) {
                                var item = items[i];
                                var nameEl = item.querySelector('[class*=\\"UserName\\"], [class*=\\"name\\"]');
                                var previewEl = item.querySelector('[class*=\\"MessagePreview\\"], [class*=\\"content\\"]');
                                var timeEl = item.querySelector('[class*=\\"time\\"]');
                                var unreadEl = item.querySelector('[class*=\\"unread\\"], [class*=\\"badge\\"]');
                                if (nameEl) {
                                    conversations.push({
    ...
    def mark_as_read(self, conv_id: str) -> Dict[str, Any]:
        ...
        """
        logger.info("Marking conversation {} as read", conv_id)
        ...
        # In Safari mode, opening the conversation marks it as read
        script = f'''
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {{
                            ...
                            for (var i = 0; i < items.length; i++) {{
                                var item = items[i];
                                if (item.getAttribute('data-conv-id') === '{conv_id}' || i === parseInt('{conv_id}')) {{
                                    item.click();
                                    return 'clicked';
                                }}
                            }}
        ...
        if success and result == "clicked":
            time.sleep(1)
            return {"success": True, "conversation_id": conv_id}
        return {"success": False, "error": f"Could not mark conversation as read: {result}"}
    ...
    def send_message(self, conv_id: str, text: str) -> Dict[str, Any]:
        ...
        logger.info("Sending message in conversation {}: {}", conv_id, text[:50])
        ...
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {{
                            var input = document.querySelector('[data-e2e=\\"chat-input\\"], [contenteditable=\\"true\\"], textarea[class*=\\"editor\\"]');
                            if (input) {{
                                input.focus();
        ...
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {
                            var sendBtn = document.querySelector('[data-e2e=\\"chat-send-btn\\"], [class*=\\"SendButton\\"], button[aria-label*=\\"Send\\"]');
        ...
        if success and result == "sent":
            ...
            logger.info("Message sent in conversation {}", conv_id)
    ...
    def read_messages(self, conv_id: str, limit: int = 50) -> Dict[str, Any]:
        ...
        """
        logger.info("Reading messages for conversation {}", conv_id)
            ...
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {{
                            var messages = [];
                            var items = document.querySelectorAll('[class*=\\"MessageItem\\"], [data-e2e=\\"chat-message\\"]');
                            for (var i = 0; i < Math.min(items.length, {limit}); i++) {{
                                var item = items[i];
                                var textEl = item.querySelector('[class*=\\"content\\"], [class*=\\"text\\"]');
                                var timeEl = item.querySelector('[class*=\\"time\\"]');
                                var isSent = item.classList.contains('sent') || item.querySelector('[class*=\\"self\\"]') !== null;
                                if (textEl) {{
                                    messages.push({{
    ...
    def new_conversation(self, username: str, message: str) -> Dict[str, Any]:
        """
        Start a new DM conversation with a TikTok user.
        ...
        logger.info("Starting new conversation with @{}", username)
        ...
        # Safari: navigate to user profile and click message
        profile_url = f"{self.TIKTOK_BASE_URL}/@{username}"
        self._navigate_to(profile_url)
        time.sleep(3)
        ...
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {
                            var msgBtn = document.querySelector('[data-e2e=\\"message-button\\"], a[href*=\\"/messages\\"]');
                            if (msgBtn) {
                                msgBtn.click();
                                return 'clicked';
                            }
        ...
        '''
        success, result = self._run_applescript(msg_btn_script)
        if not success or result == "not_found":
            return {"success": False, "error": f"Could not find message button for @{username}"}
    ...
    def send_media(self, conv_id: str, media_path: str) -> Dict[str, Any]:
        ...
        logger.info("Sending media in conversation {}: {}", conv_id, media_path)
        ...
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {
                            var attachBtn = document.querySelector('[data-e2e=\\"chat-image-btn\\"], [class*=\\"AttachButton\\"], [aria-label*=\\"image\\"], [aria-label*=\\"photo\\"]');
                            if (attachBtn) {
                                attachBtn.click();
                                return 'clicked';
                            }
        ...
        if not success or result == "not_found":
            return {"success": False, "error": "Could not find attachment button"}
        ...
        if success:
            self.messages_sent_count += 1
            self.last_message_sent_at = datetime.now(timezone.utc)
            logger.info("Media sent in conversation {}: {}", conv_id, filename)
            time.sleep(2)
    ...
    def send_sticker(self, conv_id: str, sticker_id: str) -> Dict[str, Any]:
        """
        Send a sticker in a conversation.

        Args:
            conv_id: Conversation identifier.
            sticker_id: Sticker identifier string.

        Returns:
            Dict with ``success`` flag.
        """
        if not sticker_id:
            return {"success": False, "error": "sticker_id is required"}

        logger.info("Sending sticker {} in conversation {}", sticker_id, conv_id)
        ...
        if self.dev_mode:
            ...
            conv.messages.append(msg)
            conv.last_message_preview = "[sticker]"
            conv.last_message_at = msg.timestamp
            self.messages_sent_count += 1
            return {"success": True, "message_id": msg.id, "sticker_id": sticker_id}
        ...
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {
                            var stickerBtn = document.querySelector('[data-e2e=\\"chat-sticker-btn\\"], [class*=\\"StickerButton\\"], [aria-label*=\\"sticker\\"], [aria-label*=\\"Sticker\\"]');
                            if (stickerBtn) {
                                stickerBtn.click();
                                return 'clicked';
                            }
        ...
        '''
        success, result = self._run_applescript(sticker_btn_script)
        if not success or result == "not_found":
            return {"success": False, "error": "Could not find sticker button"}

        time.sleep(1)

        # Select sticker by id or first available
        escaped_id = self._escape_for_js(sticker_id)
        select_script = f'''
        tell application "Safari"
            tell window 1
                tell current tab
                    set jsResult to do JavaScript "
                        (function() {{
                            var stickers = document.querySelectorAll('[class*=\\"StickerItem\\"], [data-sticker-id]');
                            for (var i = 0; i < stickers.length; i++) {{
                                var sid = stickers[i].getAttribute('data-sticker-id') || stickers[i].getAttribute('data-id') || '';
                                if (sid === \\"{escaped_id}\\" || i === 0) {{
                                    stickers[i].click();
                                    return 'selected';
                                }}
                            }}
        ...
        '''
        success, result = self._run_applescript(select_script)
        if success and result == "selected":
            self.messages_sent_count += 1
            self.last_message_sent_at = datetime.now(timezone.utc)
            logger.info("Sticker {} sent in conversation {}", sticker_id, conv_id)
            return {"success": True, "conversation_id": conv_id, "sticker_id": sticker_id}
        return {"success": False, "error": f"Failed to select sticker: {result}"}
...
def get_tiktok_dm_controller(dev_mode: bool = False) -> TikTokDMController:
    """Return the singleton ``TikTokDMController`` instance."""
    return TikTokDMController.get_instance(dev_mode=dev_mode)
